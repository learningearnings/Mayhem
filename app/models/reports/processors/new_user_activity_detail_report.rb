  ...
  module Processors
    class NewUserActivityDetailReport
      ...
      def run
        ...
        File.open("/tmp/student_purchases" + @filename, "w") {|f| f.write Reports::NewStudentPurchasesReport.new(options).run }                                   
        AdminMailer.delay.user_activity_detail_report(filename, @email_recipients, @options)
      end
    end
  end
end
