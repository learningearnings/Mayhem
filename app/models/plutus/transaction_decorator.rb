...
#  has_many :amounts, :class_name => "Plutus::Amount", :inverse_of => :transaction
  ...
  def self.build(hash)
    ...
    hash[:debits].each do |debit|
      ...
      debit_amounts << Plutus::DebitAmount.new(:account => a, :amount => debit[:amount], :transaction => transaction)
    end
    hash[:credits].each do |credit|
      ...
      credit_amounts << Plutus::CreditAmount.new(:account => a, :amount => credit[:amount], :transaction => transaction)
    end
    transaction.debit_amounts = debit_amounts
    transaction.credit_amounts = credit_amounts
#    transaction.save
    transaction
  end
end
