  ...
  def perform(url, username, password, district_guid)
    ...
    begin
      ...
      if district && district.has_current_versions?
        ...
        sync_attempt.students_response = sti_client.async_students(district.current_student_version).parsed_response.inspect
        sync_attempt.staff_response    = sti_client.async_staff(district.current_staff_version).parsed_response.inspect
        sync_attempt.sections_response = sti_client.async_sections(district.current_section_version).parsed_response.inspect
        sync_attempt.rosters_response  = sti_client.async_rosters(district.current_roster_version).parsed_response.inspect
        sync_attempt.schools_response  = sti_client.schools.parsed_response.inspect
      else
