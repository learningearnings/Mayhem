class BanksController < LoggedInController
  ...
  def code_lookup
    ...
    MixPanelWorker.new.track(current_user.id, 'Lookup Credit Code')
    ...
    if otu_code.present?
      if otu_code.active? && !otu_code.expired?
        ...
        if params[:student_id]
          MixPanelWorker.new.track(current_user.id, 'Deposit e-Credit')
        else
          MixPanelWorker.new.track(current_user.id, 'Enter Credit Code')
        end
  ...
  def checking_transactions
    MixPanelWorker.new.track(current_user.id, 'View Checking History')
  ...
  def savings_transactions
    MixPanelWorker.new.track(current_user.id, 'View Savings History')    
