class BanksController < LoggedInController
  ...
  def code_lookup
    ...
    @tracker.track(current_user.id, 'Lookup Credit Code')
    ...
    if otu_code.present?
      if otu_code.active? && !otu_code.expired?
        ...
        if params[:student_id]
          @tracker.track(current_user.id, 'Deposit e-Credit')
        else
          @tracker.track(current_user.id, 'Enter Credit Code')
        end
  ...
  def checking_transactions
    @tracker.track(current_user.id, 'View Checking History')
  ...
  def savings_transactions
    @tracker.track(current_user.id, 'View Savings History')    
