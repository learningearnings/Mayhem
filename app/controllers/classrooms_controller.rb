class ClassroomsController < LoggedInController
  ...
  def remove_student
    ...
    if psl = PersonSchoolLink.find_by_school_id_and_person_id(@school.id, @student.id)
      ...
      if link.delete
        ...
        MixPanelWorker.new.track(current_user.id, 'Remove Student from Classroom')
      ...
      if pscl.save
        respond_to do |format|
          format.html {
            ...
            MixPanelWorker.new.track(current_user.id, 'Add Student to Classroom')
            redirect_to classroom_path(@classroom)
          }
  ...
  def create
    ...
    if classroom_creator.success?
      MixPanelWorker.new.track(current_user.id, 'Add Classroom')
      respond_to do |format|
        format.html {
