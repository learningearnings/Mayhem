...
/***/ function(module, exports, __webpack_require__) {
	...
	var pds_tooltip_controller_1 = __webpack_require__(29);
	...
	if (w.powerSchoolDesignSystemToolkit == null) {
	    ...
	    var pds = {
	        ...
	        tooltipController: pds_tooltip_controller_1.tooltipController,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var PdsAbstractInputWidget = (function (_super) {
	    ...
	    PdsAbstractInputWidget.prototype.actionButtonClickHandler = function (event) {
	        this.fire('actionButtonEvent', event);
	    };
...
/***/ function(module, exports) {
	...
	exports.pdsUtils = {
	    ...
	    getPersistedBoolean: function (key) {
	        try {
	    ...
	    ajax: function (url, settings) {
	        ...
	        var xhrFields = settings.xhrFields || {};
	        var data = settings.data;
	        var success = settings.success;
	        var error = settings.error;
	        var complete = settings.complete;
	        var xhr = settings.xhr ? settings.xhr() : new XMLHttpRequest();
	        try {
	            ...
	            xhr.onreadystatechange = function () {
	                if (xhr.readyState === XMLHttpRequest.DONE) {
	                    if (xhr.status === 200 || xhr.status === 304) {
	                        if (success != null) {
	                            success(xhr.response, xhr, xhr.statusText);
	                        }
	                    }
	                    else {
	                        if (error != null) {
	...
	var PdsClassPicker = (function (_super) {
	    ...
	    PdsClassPicker.prototype.toggleClassSelector = function (event) {
	        ...
	        if (this.showClassSelector) {
	            ...
	            this.fire('pdsClassPickerClosed', true);
	        }
	    };
	    ...
	    PdsClassPicker.prototype.selectedClassDisplay = function (selectedClass) {
	        ...
	        if (selectedClass.schedulingTerm != null) {
	            return "" + (selectedClass.periodDay != null ? selectedClass.periodDay + ' ' : '') + selectedClass.courseName;
	        }
	        else {
	    ...
	    PdsClassPicker.prototype.getSecondaryTextForClassBlock = function (section) {
	        return section.periodDay != null ? 'Period ' + section.periodDay : '';
	    };
	...
	exports.pdsI18n = {
	    ...
	    locale: {
	        ...
	        'DATETIME_FORMATS': {
	            ...
	            'SHORTDAY': ['Sun', 'Mon', 'Tues', 'Wed', 'Thurs', 'Fri', 'Sat'],
...
/***/ function(module, exports, __webpack_require__) {
	...
	var PdsRichTextEditor = (function (_super) {
	    ...
	    PdsRichTextEditor.prototype.getConfiguration = function (options) {
	        var configuration = rich_text_editor_configurations_1.RichTextEditorConfiguration.getConfiguration(options);
	        configuration.wordcount.maxCharCount = this.dataMaxlength;
	        return configuration;
	    };
	    PdsRichTextEditor.prototype.attached = function () {
	        _super.prototype.attached.call(this);
	        this.isAttached = true;
	        this.initializeEditor(this.dataIsAdvanced, this.dataNoToolbar, this.dataCollapsibleToolbar, this.dataHideBottombar, this.dataShowCharcount);
	    };
	    PdsRichTextEditor.prototype.detached = function () {
	        this.isAttached = false;
	    };
	    PdsRichTextEditor.prototype.initializeEditor = function (dataIsAdvanced, dataNoToolbar, dataCollapsibleToolbar, dataHideBottombar, dataShowCharcount) {
	        var _this = this;
	        if (this.isAttached) {
	            var isOkayToProceed = this.areStrings(Array.from(arguments));
	            if (isOkayToProceed) {
	                var options = {
	                    isReadOnly: (this.dataIsreadonly === 'true'),
	                    showToolbar: (dataNoToolbar !== 'true'),
	                    collapsibleToolbar: (dataCollapsibleToolbar === 'true'),
	                    showBottomBar: (dataHideBottombar !== 'true'),
	                    showCharCount: (dataShowCharcount === 'true') && (dataHideBottombar !== 'true'),
	                    useAdvancedMode: (dataIsAdvanced === 'true')
	                };
	                ...
	                if (this.editor != null) {
	                    ...
	                    this.editor = CKEDITOR.replace(this.name + '-rich-text-editor', configuration);
	    ...
	    PdsRichTextEditor.prototype.areStrings = function (values) {
...
/***/ function(module, exports) {
	...
	"use strict";
	var RichTextEditorConfiguration = (function () {
	    function RichTextEditorConfiguration() {
	    }
	    RichTextEditorConfiguration.getConfiguration = function (options) {
	        ...
	        if (!options.showBottomBar) {
	            configuration.removePlugins += ',elementspath';
	        ...
	        if (options.isReadOnly) {
	            ...
	            if (options.showCharCount) {
	                configuration.wordcount.showCharCount = true;
	                configuration.wordcount.countHTML = true;
	            }
	            if (options.showToolbar) {
	                configuration.toolbarCanCollapse = options.collapsibleToolbar;
	                if (options.useAdvancedMode) {
	                    ...
	                    configuration.toolbar = [
	                        ...
	                        { name: 'font', items: ['TextColor', 'BGColor', 'FontSize'] },
	...
	exports.RichTextEditorConfiguration = RichTextEditorConfiguration;
	...
	exports.tooltipController = {
	    ...
	    init: function () {
	        ...
	        document.querySelector('body').addEventListener('mouseover', function (event) {
	            var target = event.target;
	            if (_this.pds.utils.is(event.target, '.pds-tooltip-trigger')) {
	                var tooltipContent = target.querySelector('.pds-tooltip');
	                var tooltipRectangle = tooltipContent.getBoundingClientRect();
	                var parent_1 = target;
	                var boundingElement = void 0;
	                for (var i = 0; i < 1000000; i++) {
	                    parent_1 = parent_1.parentElement;
	                    if (pdsUtils_1.pdsUtils.is(parent_1, 'body, .pds-content, .pds-modal-body, .pds-is-scrollable')) {
	                        boundingElement = parent_1;
	                        break;
	                    }
	                }
	                ...
	                var boundingRectangle = boundingElement.getBoundingClientRect();
	                if (target.classList.contains('pds-tooltip-align-right')) {
	                    if (tooltipRectangle.left < boundingRectangle.left) {
	                        target.classList.remove('pds-tooltip-align-right');
	                    }
	                }
	                else {
	                    if (tooltipRectangle.right > boundingRectangle.right) {
	                        target.classList.add('pds-tooltip-align-right');
	                    }
	                }
	                if (target.classList.contains('pds-tooltip-position-above')) {
	                    if (tooltipRectangle.top < boundingRectangle.top) {
	                        target.classList.remove('pds-tooltip-position-above');
	                    }
	                }
	                else {
	                    if (tooltipRectangle.bottom > boundingRectangle.bottom) {
	                        target.classList.add('pds-tooltip-position-above');
	                    }
	                }
	            }
	        });
