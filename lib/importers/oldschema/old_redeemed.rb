class OldRedeemed < ActiveRecord::Base
  ...
  belongs_to :old_reward_local, :foreign_key => :rewardlocalID, :class_name => "OldRewardLocal", :inverse_of => :old_redeemed
  belongs_to :old_reward_global, :foreign_key => :rewardglobalID, :class_name => "OldRewardGlobal", :inverse_of => :old_redeemeds
  belongs_to :old_point, :class_name => 'OldPoint', :foreign_key => :pointID, :inverse_of => :old_redeemed
end
